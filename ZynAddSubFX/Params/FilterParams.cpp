...
REALTYPE FilterParams::getq(){
    return(exp(powf((REALTYPE) Pq/127.0,2)*log(1000.0))-0.9);
};
...
REALTYPE FilterParams::getfreqx(REALTYPE x){
    if (x>1.0) x=1.0;
    REALTYPE octf=powf(2.0,getoctavesfreq());
...
void FilterParams::formantfilterH(int nvowel,int nfreqs,REALTYPE *freqs){
	...
	if (Pstages>0) filter_q=(filter_q>1.0 ? powf(filter_q,1.0/(Pstages+1)) : filter_q);
...
REALTYPE FilterParams::getformantamp(unsigned char amp){
    REALTYPE result=powf(0.1,(1.0-amp/127.0)*4.0);
    return(result);
};

REALTYPE FilterParams::getformantq(unsigned char q){
    //temp
    REALTYPE result=powf(25.0,(q-32.0)/64.0);
    return(result);
};
