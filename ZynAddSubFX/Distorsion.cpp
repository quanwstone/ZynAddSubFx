...
void waveshapesmps(int n,REALTYPE *smps,unsigned char type,unsigned char drive){
    ...
    switch(type){
	case 1:	ws=powf(10,ws*ws*3.0)-1.0+0.001;//Arctangent
		for (i=0;i<n;i++) 
	...
	case 6:	ws=ws*ws*ws*32+0.0001;//Zigzag
		...
		break;
	case 7:	ws=powf(2.0,-ws*ws*8.0); //Limiter
		for (i=0;i<n;i++) {
		    REALTYPE tmp=smps[i];
		    if (fabs(tmp)>ws) {
		...
		break;
	case 8:	ws=powf(2.0,-ws*ws*8.0); //Upper Limiter
		for (i=0;i<n;i++) {
		    REALTYPE tmp=smps[i];
		    if (tmp>ws) smps[i]=ws;
		    smps[i]*=2.0;
		...
		break;
	case 9:	ws=powf(2.0,-ws*ws*8.0); //Lower Limiter
		for (i=0;i<n;i++) {
		    REALTYPE tmp=smps[i];
		    if (tmp<-ws) smps[i]=-ws;
		    smps[i]*=2.0;
		...
		break;
	case 10:ws=(powf(2.0,ws*6.0)-1.0)/powf(2.0,6.0); //Inverse Limiter
		for (i=0;i<n;i++) {
		    REALTYPE tmp=smps[i];
		    if (fabs(tmp)>ws) {
		...
		break;
	case 11:ws=powf(5,ws*ws*1.0)-1.0;//Clip
		for (i=0;i<n;i++) 
	...
	case 13:ws=ws*ws*ws*32.0+0.0001;//Pow2
		...
		break;
	case 14:ws=powf(ws,5.0)*80.0+0.0001;//sigmoid
		if (ws>10.0) tmpv=0.5;
...
void Distorsion::setlpf(unsigned char Plpf){
    this->Plpf=Plpf;
    REALTYPE fr=exp(powf(Plpf/127.0,0.5)*log(25000.0))+40;
    lpfl->setfreq(fr);
    lpfr->setfreq(fr);
};
...
void Distorsion::sethpf(unsigned char Phpf){
    this->Phpf=Phpf;
    REALTYPE fr=exp(powf(Phpf/127.0,0.5)*log(25000.0))+20.0;
    hpfl->setfreq(fr);
    hpfr->setfreq(fr);
};
